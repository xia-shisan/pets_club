<template>
  <van-list
    ...
    @load="onLoad"
  >
    <div v-for="item in postComment.list" :key="item.id">
      <!-- 评论用户信息 -->
      <van-cell
        :title="item.nickname"
        label="初级粉丝"
        ...
        >
          <template #right-icon>
            <van-tag
              color="#eeeeee"
              class="reply-tag"
              @click="lookReply(item.post_id, item.id)"
            >
              {{ item.child_count }}回复
            </van-tag>
          </template>
...
export default {
  ...
  methods: {
    ...
    onLoad() {
      ...
      if (this.$parent.page <= this.postComment.last) {
        // 显示加载提示
        this.$toast.loading({
          message: '加载中...',
          forbidClick: true,
