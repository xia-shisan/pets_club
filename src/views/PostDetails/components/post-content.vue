<template>
  ...
  <div>
    ...
    <van-cell
      ...
      :label="postDetails.create_time | formatDate('yyyy-MM-dd hh:mm:ss')"
      label-class="date"
      :border="false"
    />
    ...
    <van-cell
      class="pet-info"
      :border="false"
      @click="lookPetDetails(postDetails.pet_id)"
    >
      <template #icon>
        <van-tag color="#add8e6" round class="pet-img">
          <van-image
            ...
            :src="petImg"
          />
          <span>{{ postDetails.pet_name }}</span>
        </van-tag>
      </template>
    </van-cell>
    <!-- 标签 -->
    <van-cell
      :title="postDetails.label"
      center
    ...
    <!-- 王国 -->
    <van-cell
      :title="postDetails.category_name + '王国'"
      :label="postDetails.category_label"
      title-class="title-weight"
      center
      is-link
      class="kingdom"
      @click="jumpLevelDetails(postDetails.category_id)"
    >
      <!-- 王国图片 -->
      <template #icon>
        <van-image
          ...
          radius="5px"
          class="img-mr"
          :src="postDetails.category_image"
        />
      </template>
    </van-cell>
  </div>
...
export default {
  ...
  computed: {
    // 发帖用户名
    nickname() {
      return this.postDetails.nickname ? this.postDetails.nickname : '未知'
    },
    // 发帖用户头像
    userImg() {
      return this.postDetails.avatar
        ? this.postDetails.avatar
        : this.defaultPhoto
    },
    // 发帖用户的宠物名
    petName() {
      return this.postDetails.pet_name ? this.postDetails.pet_name : ''
    },
    // 宠物头像
    petImg() {
      return this.postDetails.pet_image
        ? this.postDetails.pet_image
